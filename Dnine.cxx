...
// // full fixed n_waves = {1,2,1,1,3,1,1,3,1,};
	...
	if (!integral->loadIntegrals("./integralFiles/ps_"+integral_file_name+"_regular.dat","./integralFiles/ac_"+integral_file_name+"_regular.dat")) {
		...
		std::cout << "Dnine::main(...): INFO: Loaded: " << "'./integralFiles/ps_"+integral_file_name+"_regular.dat' and './integralFiles/ac_"+integral_file_name+"_regular.dat'" <<std::endl;
	}
...
//#define DO_ZERO_MODE_PRIORS
	...
	size_t countZero = 0;
	while (true) {
		std::string zeroFileName = "./zeroModeFiles/"+freeString+"_"+std::to_string(countZero)+".dat";
...
//		BELLE_result_values.push_back(std::complex<double>(pow(nData * total_bg_coefficient/integral_bg->totalIntensity(std::vector<std::complex<double> >(1,std::complex<double>(1.,0.)), true), .5),0.));
	...
	outFileName += "_"+std::to_string(bestLL)+"_"+std::to_string(seed)+".dat";
	outFile.open(outFileName.c_str());
	outFile << std::setprecision(std::numeric_limits<double>::digits10 + 1);
