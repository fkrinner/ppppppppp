...
double integrator::multiplyLR(const std::vector<std::complex<double> >& L, const std::vector<std::complex<double> >& R, bool accCorr) const {
	if (not _isIntegrated) {
		std::cerr << "integrator::totalIntensity(...): ERROR: Not integrated yet. Returning zero." << std::endl;
		return 0.;
	}
...
//	}
	double alternative = 0.;
	const double *reImL = (double*)&L[0]; // Trust this at the moment
	const double *reImR = (double*)&R[0]; // Trust this at the moment
	const std::vector<std::vector<double> > &realMatrix = accCorr ? _realAccCorrMatrix : _realIntegralMatrix;
	for (size_t a_i = 0; a_i < 2*_nAmpl; ++a_i) {
		for (size_t a_j = 0; a_j < 2*_nAmpl; ++a_j) {
//			if ( reIm[a_i]*realMatrix[a_i][a_j]*reIm[a_j] != 0.) {
//				std::cout << a_i << " " << a_j << "innse " << reIm[a_i]<< " " <<realMatrix[a_i][a_j] << " " << reIm[a_j] << std::endl;
//			}
			alternative += reImL[a_i]*realMatrix[a_i][a_j]*reImR[a_j];
		}
	}
	return alternative;	
}
std::vector<double> integrator::DLmultiplyLR  (const std::vector<std::complex<double> >& L, const std::vector<std::complex<double> >& R, bool accCorr) const {
// Return a vector of length 2*nAmpl() for the derivative w.r.t. re, and im : {dRe0, dIm0, dRe1, ..., dImnAmpl()}
	if (not _isIntegrated) {
		std::cerr << "integrator::DtotalIntensity(...): ERROR: Not integrated yet. Returning empty vector." << std::endl;
		return std::vector<double>();
	...
	std::vector<double> retVal(2*nAmpl(), 0.);
	const std::vector<std::vector<double> > &matrix = accCorr ? _realAccCorrMatrix : _realIntegralMatrix;
//	std::cout << "fsaddfas " << matrix.size() << std::endl;
//	for (std::vector<double> a : matrix) {
//		std::cout << " brsho " << a.size() << " AA " << _nAmpl<< "  | " << L.size() << " " << R.size() <<  std::endl;
//	}
	const double* reImR=  (double*)&R[0];
	for (size_t a_i = 0; a_i < 2*_nAmpl; ++a_i) {
		for (size_t a_j = 0; a_j < 2*_nAmpl; ++ a_j) {
			retVal[a_i] += matrix[a_i][a_j]*reImR[a_j];
		}
	}
	return retVal;
}

std::vector<double> integrator::DRmultiplyLR  (const std::vector<std::complex<double> >& L, const std::vector<std::complex<double> >& R, bool accCorr) const{
// Return a vector of length 2*nAmpl() for the derivative w.r.t. re, and im : {dRe0, dIm0, dRe1, ..., dImnAmpl()}
	if (not _isIntegrated) {
		std::cerr << "integrator::DtotalIntensity(...): ERROR: Not integrated yet. Returning empty vector." << std::endl;
		return std::vector<double>();
	...
	std::vector<double> retVal(2*nAmpl(), 0.);
	const std::vector<std::vector<double> > &matrix = accCorr ? _realAccCorrMatrix : _realIntegralMatrix;
	const double* reImL=  (double*)&L[0];
	for (size_t a_i = 0; a_i < 2*_nAmpl; ++a_i) {
		for (size_t a_j = 0; a_j < 2*_nAmpl; ++ a_j) {
			retVal[a_j] += reImL[a_i]*matrix[a_i][a_j];
		}
	}
	return retVal;
}
...
bool integrator::setNonDiagToZero() {
	for (size_t a1 = 0; a1 < _nAmpl; ++ a1) {
		for (size_t a2 = 0; a2 < _nAmpl; ++a2) {
			if (a1 == a2) {
				continue;
			}
			_integralMatrix[a1][a2] = std::complex<double>(0.,0.);
			_accCorrIntegralMatrix[a1][a2] = std::complex<double>(0.,0.);
		}
	}
	return true;
}

bool integrator::resize(size_t nAmpl) {
	if (nAmpl > _nAmpl) {
		std::cout << "integrator::resize(...): ERROR: Cannot enlargen the matrix with resize(...)" << std::endl;
		return false;
	}
	_amplitudeCoherenceBorders = {0,nAmpl}; // Make everything coherent now
	_amplitudes.resize(nAmpl);
	_integralMatrix.resize(nAmpl);
	_accCorrIntegralMatrix.resize(nAmpl);
	for (size_t a = 0; a < nAmpl; ++a) {
		_integralMatrix[a].resize(nAmpl);
		_accCorrIntegralMatrix[a].resize(nAmpl);
	}
	_nAmpl = nAmpl;
	if (!makeRealMatrices()) {
		std::cout << "integrator::resize(...): ERROR: Could nor create the real matrices" << std::endl;
		return false;
	}
	return true;
}
